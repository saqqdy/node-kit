 ...
 */
export async function yarnWorkspaceRoot(cwd: string = process.cwd()): Promise<string | null> {
	let previous = null,
 ...
 * @returns result - workspace root dir
 */
export function yarnWorkspaceRootSync(cwd: string = process.cwd()): string | null {
	let previous = null,
		current = normalize(cwd)
