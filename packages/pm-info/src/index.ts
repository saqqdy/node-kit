 ...
 */
export async function pmInfo(cwd: string = process.cwd()): Promise<PackageManager | null> {
 ...
 * @returns result - WorkspaceRootResult | null
 */
export function pmInfoSync(cwd: string = process.cwd()): PackageManager | null {
	const NODE_MODULES_PATH = join(cwd, 'node_modules')
 ...
 */
export function normalizePMSpec(pm: string): PackageManager {
	if (pm[0] === '@') {
